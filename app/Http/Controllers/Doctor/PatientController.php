...
class PatientController extends Controller
{
    ...
    {
        if ($record->patient_id !== $patient->id) abort(403);
        if ($record->doctor_id !== auth('doctor')->id()) abort(403);
    ...
    private function notifyReception(Patient $patient, $doctor, TreatmentRecord $record): void
    {
        ...
        $staffUsers = User::whereHas('role', fn($q) => $q->whereIn('name', ['receptionist', 'admin']))
            ->when($doctor->branch_id, fn($q) => $q->where(fn($q2) =>
                $q2->where('branch_id', $doctor->branch_id)->orWhereNull('branch_id')
            ))
            ->get();
    ...
    {
        if ($media->patient_id !== $patient->id) abort(403);
        Storage::disk('public')->delete($media->file_path);
        $media->delete();
        return back()->with('success', 'Зураг устгагдлаа.');
    }

    public function pollOrthoVisits(Patient $patient): \Illuminate\Http\JsonResponse
    {
    ...
    public function pollGeneralVisits(Patient $patient): \Illuminate\Http\JsonResponse
    {
