...
const TICKER = ['Гажиг засал', 'Имплант', 'Цайруулалт', 'Дүүргэлт', '3D сканнер', 'Хүүхдийн эмчилгээ', 'Протез', 'Цэвэрлэгээ'];
...
export default function Welcome({ doctors = [], treatments = [], gallery = [], branches = [], faqs = [], articles = [], stats = { doctors: 0, appointments: 0, branches: 0 }, reels = [] }: PageProps) {
    ...
    return (
        <PublicLayout heroOverlay editorial>
            ...
            {/* ── ГҮЙДЭГ МӨР ───────────────────────────────────────────────── */}
            <section className="cw-sec-tight">
                <Bleed className="cw-ticker" tone="light">
                    <div className="cw-ticker-track" aria-hidden="true">
                        {[0, 1].map((dup) => TICKER.map((t) => (
                            <span key={`${dup}-${t}`}><i />{t}</span>
                        )))}
                    </div>
                </Bleed>
            </section>
