...
import { CheckCircle2, ChevronDown, ChevronUp, Smile } from 'lucide-react';
...
/* ── Mobile bonus card (premium look) ───────────────────── */
    ...
    return (
        <div style={{ background: 'var(--my-card-bg)', borderRadius: 22, overflow: 'hidden', boxShadow: 'var(--my-shadow)', border: '1px solid var(--my-card-border)' }}>
            <button onClick={() => setOpen(v => !v)}
                ...
                {/* Month badge */}
                <div style={{ width: 50, height: 50, borderRadius: 16, background: 'linear-gradient(145deg, #faf5ff, #f3e8ff)', border: '1.5px solid #e9d5ff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexShrink: 0, gap: 1 }}>
                    <span style={{ fontSize: 22, fontWeight: 900, color: VIO, lineHeight: 1 }}>{entry.month}</span>
                    <span style={{ fontSize: 9, fontWeight: 700, color: VIO2, letterSpacing: 0.3 }}>{entry.year}</span>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                    <div style={{ textAlign: 'right' }}>
                        <p style={{ fontSize: 16, fontWeight: 900, color: VIO, margin: 0, letterSpacing: -0.3 }}>{fmtMoney(entry.total_amount)}</p>
                        <p style={{ fontSize: 9, color: 'var(--my-faint)', margin: '2px 0 0', fontWeight: 600 }}>УРАМШУУЛАЛ</p>
                    </div>
                    <div style={{ width: 26, height: 26, borderRadius: '50%', background: open ? '#faf5ff' : 'var(--my-pill-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {open ? <ChevronUp size={13} color={VIO} /> : <ChevronDown size={13} color="#999" />}
                    </div>
                </div>
            </button>
            ...
            {open && (
                <div style={{ borderTop: '1px solid var(--my-divider)', padding: '14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {/* Violet summary block */}
                    <div style={{ background: `linear-gradient(135deg, ${VIO3} 0%, ${VIO2} 60%, ${VIO} 100%)`, borderRadius: 18, padding: '16px 18px' }}>
...
/* ── Desktop card (kept simple but consistent) ─────────── */
...
export default function MyReceptionBonus({ employee, entries, criteria }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Ресепшний урамшуулал', href: '/my/reception-bonus' },
    ];
    ...
    return (
        <MyLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="md:hidden" style={{ flex: 1, background: 'var(--my-page-bg)', overflowY: 'auto', WebkitOverflowScrolling: 'touch', paddingBottom: 'calc(88px + env(safe-area-inset-bottom,0px))' } as React.CSSProperties}>
                {/* ═══ VIOLET HERO ════════════════════════════════════ */}
                <div style={{ background: `linear-gradient(160deg, #a855f7 0%, ${VIO} 30%, ${VIO2} 65%, ${VIO3} 100%)`, position: 'relative', overflow: 'hidden' }}>
                        ...
                        <Link href="/my/profile" style={{ textDecoration: 'none', flexShrink: 0 }}>
                            <div style={{ width: 36, height: 36, borderRadius: '50%', overflow: 'hidden', border: '2px solid rgba(255,255,255,0.5)', background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                {employee.photo_url
                                    ...
                                    : <span style={{ fontSize: 12, fontWeight: 800, color: 'white' }}>{employee.initials ?? (employee.full_name?.[0] ?? 'Р')}</span>
                                }
                            </div>
                        </Link>
                    </div>
            ...
            <div className="hidden md:flex flex-col gap-4 p-4 md:p-6">
                <div>
                    <h1 className="text-lg font-bold text-foreground">Ресепшний урамшуулал</h1>
