...
import {
    ...
    Clock, DollarSign, FileSpreadsheet, Plus, Save, Send, Trash2, X,
} from 'lucide-react';
...
interface Props { runs: PayrollRun[]; branches: { id: number; name: string }[] }
...
function StatusBadge({ run }: { run: PayrollRun }) {
    ...
    if (run.sent_entries_count > 0 && run.sent_entries_count === run.entries_count && run.status === 'draft') return (
        <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[11px] font-semibold bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-400">
            <Send className="size-3" /> Бүгд илгээсэн
        </span>
    );
    return (
        ...
        <form onSubmit={submit}>
            ...
            <div className="flex items-center justify-end gap-2 px-5 py-4 border-t bg-muted/20">
                ...
                <button type="submit" disabled={form.processing || !form.data.branch_id}
                    className="flex items-center gap-2 rounded-xl bg-foreground px-5 py-2 text-sm font-bold text-background hover:opacity-90 disabled:opacity-40 transition-all">
                    {form.processing
...
export default function PayrollIndex({ runs, branches }: Props) {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="hidden md:flex flex-col gap-5 p-4 md:p-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-xl font-bold text-foreground">Цалингийн тооцоо</h1>
                        <p className="text-sm text-muted-foreground mt-0.5">Сард 2 удаа цалингийн задаргаа бүртгэх</p>
                    </div>
                    <button onClick={() => setCreateOpen(true)}
                        className="flex items-center gap-2 rounded-xl bg-foreground px-4 py-2 text-sm font-semibold text-background hover:opacity-90 transition-opacity">
                ...
                {runs.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed bg-card py-20 text-center">
                        ...
                        <p className="text-sm font-medium text-muted-foreground">Цалингийн тооцоо байхгүй байна</p>
                        <button onClick={() => setCreateOpen(true)} className="mt-4 text-xs text-primary underline underline-offset-2">
                            Шинэ тооцоо үүсгэх
                        </button>
                    </div>
                ) : (
                                ...
                                {runs.map(r => (
                                    <tr key={r.id} className="group hover:bg-muted/20 transition-colors cursor-pointer"
                                        ...
                                        <td className="px-5 py-3.5">
                                            ...
                                            <p className="text-xs text-muted-foreground mt-0.5">{r.half === 'first' ? '1–15' : '16–31'}
                                                <span className="sm:hidden ml-2"><StatusBadge run={r} /></span>
                                            </p>
                                        </td>
                                        <td className="px-4 py-3.5 hidden sm:table-cell"><StatusBadge run={r} /></td>
                                        <td className="px-4 py-3.5 text-center hidden md:table-cell">
                                            <span className="text-xs font-medium text-foreground">{r.entries_count}</span>
                                        ...
                                        <td className="px-4 py-3.5 hidden lg:table-cell">
                                            ...
                                            {r.created_by && <p className="text-[11px] text-muted-foreground/60">{r.created_by}</p>}
                                        </td>
                                        <td className="px-4 py-3.5 text-right">
                                            <div className="flex items-center justify-end gap-1.5" onClick={e => e.stopPropagation()}>
                                                <a href={`/hr/payroll/${r.id}/excel`}
                                                    className="flex items-center gap-1 rounded-lg border px-2.5 py-1.5 text-[11px] font-medium text-muted-foreground hover:bg-muted transition-colors">
