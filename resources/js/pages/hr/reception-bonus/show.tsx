...
const KEYS = ['registrations','calls_received','call_reminders','complaints','compliments','hubspot_regs','payments'] as const;
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl overflow-hidden" style={{ boxShadow: '0 2px 10px rgba(0,0,0,0.06)' }}>
            <button onClick={() => setOpen(v => !v)}
                ...
                <div className="flex items-center gap-3">
                    <div className="size-9 rounded-full bg-violet-100 dark:bg-violet-950/40 flex items-center justify-center shrink-0">
                        <span className="text-xs font-bold text-violet-600 dark:text-violet-400">{entry.name.slice(0,2)}</span>
                    </div>
                    <div>
            ...
            {open && (
                <div className="border-t border-gray-100 dark:border-zinc-800 px-4 py-4 space-y-3">
                            ...
                            return (
                                <div key={key} className={`flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-zinc-700 last:border-0 ${i % 2 === 0 ? '' : 'bg-white/50 dark:bg-zinc-700/30'}`}>
                                    ...
                                    <div className="flex items-center gap-3 shrink-0">
                                        ...
                                        <span className={`w-20 text-right text-xs tabular-nums font-semibold ${amount < 0 ? 'text-red-600' : amount > 0 ? 'text-gray-700 dark:text-gray-300' : 'text-gray-300'}`}>
                    ...
                    {!isFinal && !entry.is_sent && (
                        <button onClick={() => router.post(`/hr/reception-bonus/${runId}/entries/${entry.id}/send`)}
...
export default function ReceptionBonusShow({ run, entries: initial, criteria }: Props) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: run.title, href: `/hr/reception-bonus/${run.id}` },
    ];
    ...
    function save() {
        setSaving(true);
        router.put(`/hr/reception-bonus/${run.id}`, { entries: entries.map(e => ({ ...e })) }, {
            onSuccess: () => setSaved(true),
            onFinish:  () => setSaving(false),
        });
    }
    function finalize() { router.patch(`/hr/reception-bonus/${run.id}/finalize`, { entries: entries.map(e => ({ ...e })) }); }
    function reopen()   { router.patch(`/hr/reception-bonus/${run.id}/reopen`); }
    ...
    function StatusBadge() {
        ...
        return (
            <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[11px] font-semibold bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400">Ноорог</span>
        );
    }
            ...
            <div className="md:hidden min-h-full bg-[#f2f2f7] dark:bg-zinc-950">
                ...
                <div className="sticky top-0 z-30 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-sm border-b border-gray-100 dark:border-zinc-800 px-4 py-3">
                    <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                            <a href={`/hr/reception-bonus/${run.id}/excel`} target="_blank"
                ...
                <div className="px-4 pt-4 pb-10 space-y-3">
                    <p className="text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-zinc-400 px-1">
                        {entries.length} ресепшн ажилтан
                    </p>
                    {entries.map((e, idx) => (
            ...
            <div className="hidden md:flex flex-col gap-4 p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">{entries.length} ресепшн ажилтан</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <a href={`/hr/reception-bonus/${run.id}/excel`} target="_blank"
                                                ...
                                                {e.is_sent
                                                    ...
                                                    : !isFinal && (
                                                        <button title="Энэ ажилтанд илгээх"
                                                            onClick={() => router.post(`/hr/reception-bonus/${run.id}/entries/${e.id}/send`)}
