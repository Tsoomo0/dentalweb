...
import {
    ...
    ChevronRight, Clock, Menu, Monitor, Plus, Search, User, UserCheck, X, XCircle,
} from 'lucide-react';
...
export default function AppointmentsIndex({ appointments: initialApts, doctors, branches, treatments, creators, stats: initialStats }: Props) {
    ...
    const visibleApts = useMemo(() => {
        ...
        if (filterBranches.size > 0) {
            ...
            r = r.filter(a =>
                (a.branch_id !== null && filterBranches.has(Number(a.branch_id))) ||
                (a.doctor_id !== null && docIds.has(Number(a.doctor_id)))
            );
        }
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            {/* Modals */}
            {createDate && (
                <AptFormModal routePrefix="/admin/appointments"
            ...
            {isMobile ? (
                ...
                ══════════════════════════════════════════════════════ */
                <div className="bg-background text-foreground" style={{ display:'flex', flexDirection:'column', height:'calc(100svh - 4rem)', overflow:'hidden' }}>

                    {/* ── HEADER ── */}
                    <div className="bg-background border-b border-border" style={{ flexShrink:0, padding:'10px 16px 0', userSelect:'none' }}>
                        {/* Top row: hamburger · date · view toggle */}
                                                    ...
                                                    {cols.map(({ apt: a, col, totalCols }) => {
                                                        ...
                                                        return (
                                                            <div key={a.id} onClick={() => openApt(a)}
                                                                ...
                                                                {/* Doctor avatar — only for taller cards */}
                                                                {!compact && (
                                                                    <div style={{
                        ...
                        {view === 'month' && (
                            <div style={{ height:'100%', overflowY:'auto', display:'flex', flexDirection:'column' }}>
                                ...
                                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 16px 4px', flexShrink:0 }}>
                                    <button onClick={prevMonth} style={{ padding:6, borderRadius:8, background:'rgba(255,255,255,0.07)', border:'none', cursor:'pointer' }}>
                                        <ChevronLeft style={{ width:18, height:18, color:'white' }} />
                                    </button>
                                    <button onClick={() => { setYear(today.getFullYear()); setMonth(today.getMonth()); changeSelected(todayStr); }}
                                        style={{ fontSize:14, fontWeight:700, color:'white', background:'none', border:'none', cursor:'pointer' }}>
                                        {year} · {MONTHS_MN[month]}
                                    </button>
                                    <button onClick={nextMonth} style={{ padding:6, borderRadius:8, background:'rgba(255,255,255,0.07)', border:'none', cursor:'pointer' }}>
                                ...
                                <div style={{ flex:1, borderTop:'1px solid rgba(255,255,255,0.07)', padding:'10px 16px 16px', overflowY:'auto' }}>
                        ...
                        {view === 'week' && (
                            <div style={{ height:'100%', overflowY:'auto', overflowX:'hidden' }}>
                                {(() => {
                                    ...
                                    return (
                                        <>
                                            {/* Time grid — header is handled by the shared week strip above */}
                                            <div style={{ position:'relative', display:'flex', height: totalH }}>
                    ...
                    {/* ── BOTTOM NAV ── */}
                    <div className="bg-card border-t border-border" style={{ flexShrink:0, display:'flex' }}>
                        {([
                            { icon: CalendarCheck2, label:'Календарь' },
                            { icon: User,           label:'Эмч нар' },
                            { icon: Search,         label:'Хайх' },
                            { icon: Monitor,        label:'Профайл' },
                        ] as const).map(({ icon: Icon, label }, ti) => (
                            <button key={label}
                                onClick={() => {
                                    if (ti === 1) setDoctorPanelOpen(true);
                                    if (ti === 2) router.get('/admin/appointments/search');
                                    if (ti === 3) router.get('/admin/dashboard');
                                }}
                                style={{
                                    flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:3,
                                    padding:'10px 0 12px', background:'none', border:'none', cursor:'pointer',
                                    color: ti === 0 ? '#22c55e' : 'hsl(var(--muted-foreground))',
                                }}>
                                    ...
                                    {sidebarDoctors.map(d => {
                                        ...
                                        return (
                                            <button key={d.id} onClick={() => toggleDoc(d.id)}
                                                ...
                                                {/* Checkbox — same accent color as desktop */}
                                                <div style={{
                                                ...
                                                {/* Avatar — photo if available, initials fallback (same as desktop) */}
                                                <div style={{
                    ...
                    <div className="relative shrink-0 flex items-center gap-2">
                        ...
                        <div className="flex flex-1 items-center gap-2 rounded-xl border bg-card shadow-sm px-3 py-2">
                            ...
                            <input
                                ...
                                onKeyDown={e => { if (e.key === 'Enter') { const p: Record<string,string> = {}; if (searchQuery) p.q = searchQuery; if (searchCreator) p.created_by = searchCreator; if (searchDateFrom) p.date_from = searchDateFrom; if (searchDateTo) p.date_to = searchDateTo; router.get('/admin/appointments/search', p); } }}
                        ...
                        <button
                            onClick={() => { const p: Record<string,string> = {}; if (searchQuery) p.q = searchQuery; if (searchCreator) p.created_by = searchCreator; if (searchDateFrom) p.date_from = searchDateFrom; if (searchDateTo) p.date_to = searchDateTo; router.get('/admin/appointments/search', p); }}
                                    ...
                                    <div ref={dayScrollRef} className="cal-scroll flex-1 overflow-auto">
                                        <div style={{ minWidth: '100%', width: 56 + 7 * WCOL_MIN }}>
                                            ...
                                            <div className="relative flex" style={{ height: wTotalH }}>
                                                ...
                                                {/* Shared hour/half-hour grid lines spanning full width */}
                                    ...
                                    <div ref={dayScrollRef} className="cal-scroll flex-1 overflow-auto">
                                        ...
                                        <div style={{ minWidth: '100%', width: 56 + dayDoctors.length * COL_MIN, minHeight: totalH + 90 }}>
                                            ...
                                            <div className="relative flex" style={{ height: totalH }}>
                                                ...
                                                {/* Shared time-grid lines spanning full width */}
                                                <div className="pointer-events-none absolute inset-0">
                                                    {Array.from({length: HOUR_END - HOUR_START}, (_, i) => (
                                                        <div key={i} style={{ position:'absolute', left:0, right:0, top: i * HOUR_H, borderTop:'1px solid var(--cal-line-hour)' }} />
                                                    ))}
