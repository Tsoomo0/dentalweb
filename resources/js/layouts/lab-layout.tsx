...
import { Eye, EyeOff, FlaskConical, KeyRound, LayoutGrid, UserRound, X } from 'lucide-react';
...
const BOTTOM_NAV = [
    { icon: LayoutGrid,   label: 'Самбар',     href: '/lab/dashboard' },
    { icon: FlaskConical, label: 'Лаб бүртгэл', href: '/lab/lab-orders' },
];
...
export default function LabLayout({ children, breadcrumbs = [] }: Props) {
                ...
                <nav className="bg-card border-t border-border"
                    ...
                    {BOTTOM_NAV.map(({ icon: Icon, label, href }) => {
                        const active = url === href
                            || url.startsWith(href + '?')
                            || (href !== '/lab/dashboard' && url.startsWith(href));
                        return (
                            <Link key={href} href={href}
                                style={{
                                    ...
                                    textDecoration: 'none',
                                    color: active ? '#7c3aed' : 'hsl(var(--muted-foreground))',
                                }}>
