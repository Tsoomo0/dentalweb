...
function NotifContent({ n }: { n: NotifItem }) {
    ...
    switch (n.notif_type) {
        ...
        case 'OrthoSignatureRequested':
            return <a href="/patient/ortho-signatures" className="block"><p className="text-xs font-semibold text-gray-800 dark:text-gray-100 leading-snug">Гажиг заслын үзлэг — гарын үсэг хүлээгдэж байна</p><p className="text-[11px] text-muted-foreground mt-0.5">{d.doctor_name}{d.visit_date ? ` · ${d.visit_date}` : ''}</p><p className="text-[11px] text-violet-600 dark:text-violet-400 mt-0.5">Портал руу орж гарын үсэг зурна уу →</p></a>;
        case 'GeneralVisitSignatureRequested':
            return <a href="/patient/ortho-signatures" className="block"><p className="text-xs font-semibold text-gray-800 dark:text-gray-100 leading-snug">Ерөнхий эмчилгээний үзлэг — гарын үсэг хүлээгдэж байна</p><p className="text-[11px] text-muted-foreground mt-0.5">{d.doctor_name}{d.visit_date ? ` · ${d.visit_date}` : ''}</p><p className="text-[11px] text-blue-600 dark:text-blue-400 mt-0.5">Портал руу орж гарын үсэг зурна уу →</p></a>;
        ...
        case 'TreatmentSentToReception':
            return <a href="/reception/treatment-payments" className="block"><p className="text-xs font-semibold text-gray-800 dark:text-gray-100 leading-snug">Эмчилгээний тооцоо ирлээ — {d.patient_name}</p><p className="text-[11px] text-muted-foreground mt-0.5">{d.doctor_name}{d.appointment_number ? ` · ${d.appointment_number}` : ''}</p>{d.amount ? <p className="text-[11px] text-cyan-600 dark:text-cyan-400 mt-0.5 font-semibold">{d.amount.toLocaleString()}₮</p> : null}</a>;
        case 'AppointmentBookedPatient':
            return <a href="/patient/appointments" className="block"><p className="text-xs font-semibold text-gray-800 dark:text-gray-100 leading-snug">Цаг захиалга бүртгэгдлээ — {d.appointment_number}</p><p className="text-[11px] text-muted-foreground mt-0.5">{[d.appointment_date, d.appointment_time, d.doctor_name, d.branch_name].filter(Boolean).join(' · ')}</p></a>;
        case 'AppointmentConfirmedPatient':
            return <a href="/patient/appointments" className="block"><p className="text-xs font-semibold text-gray-800 dark:text-gray-100 leading-snug">Цаг захиалга баталгаажлаа ✓ — {d.appointment_number}</p><p className="text-[11px] text-muted-foreground mt-0.5">{[d.appointment_date, d.appointment_time, d.doctor_name, d.branch_name].filter(Boolean).join(' · ')}</p></a>;
        ...
        default:
            return <><p className="text-xs font-semibold text-gray-800 dark:text-gray-100 leading-snug">Шинэ цаг захиалга — {d.patient_name}{d.appointment_type === 'online' ? ' (онлайн)' : ' (биечлэн)'}</p><p className="text-[11px] text-muted-foreground mt-0.5">{[d.appointment_number, d.doctor_name, d.branch_name, d.appointment_date, d.appointment_time].filter(Boolean).join(' · ')}</p></>;
    }
}
